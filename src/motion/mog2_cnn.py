...
"""
MOG2 + CNN (MobileNetV3-Small) "FP-safe" clip gating + utilities.

- Motion mask: OpenCV BackgroundSubtractorMOG2
- Optional CNN gating: crop proposals from motion mask, then MobileNetV3-Small
- Clip shaping: pre-roll / post-roll and "stay" score (motion EMA + presence latch)

This is designed to mirror the structure of your ViBe+MHI+CNN evaluator,
but swaps ViBe for MOG2 and keeps the same CNN+gate philosophy.
"""
...
class MOG2Config:
    ...
    # Shadow handling
    keep_shadows_as_fg: bool = False  # if False, shadow pixels are treated as background

    # Learning rate (passed to apply)
    learning_rate: float = -1.0       # -1 = OpenCV default adaptive

    # Optional post-filtering of raw mask (before global postprocess)
    threshold: int = 127              # threshold raw mask to fg (only used when keep_shadows_as_fg=True)


class MOG2Motion:
    def __init__(self, cfg: MOG2Config):
        self.cfg = cfg
    ...
    def apply(self, gray: np.ndarray) -> np.ndarray:
        """
        Returns a uint8 mask in {0,255}.
        """
        raw = self.bgs.apply(gray, learningRate=float(self.cfg.learning_rate))
        if raw is None:
            return np.zeros_like(gray, dtype=np.uint8)

        # raw is typically 0 (bg), 255 (fg), and 127 (shadow) if enabled
        if bool(self.cfg.detect_shadows) and (not bool(self.cfg.keep_shadows_as_fg)):
            fg = (raw == 255).astype(np.uint8) * 255
        else:
            thr = int(self.cfg.threshold)
            fg = (raw >= thr).astype(np.uint8) * 255
...
class CNNConfig:
    ...
    p_start_high: float = 0.80                # refresh even without motion
...
# =============================================================================
# Presence latch + Hybrid gate (returns a reason for logging)
# =============================================================================
...
class HybridGateConfig:
    ...
    # Motion START
    start_metric: str = "mog2_fg_pixels"
    start_threshold: float = 800.0
    ...
    cnn_start_motion_metric: str = "mog2_fg_ratio"
...
class HybridClipGate:
    ...
    def step(self, scores: Dict[str, float], cnn_p_frame: float) -> Tuple[bool, Optional[int], str]:
        """
        Returns: (recording, backfill_k, reason)
          reason in {"none","mog2_start","cnn_start","stay","post_roll"}
        """
        if not self.cfg.enabled:
        ...
        if not self.recording:
            ...
            if motion_ok or cnn_ok:
                ...
                return True, backfill_k, ("cnn_start" if cnn_ok else "mog2_start")
        ...
        if stay_val >= float(self.cfg.stay_threshold):
            self._post_left = self.post_roll_frames
            return True, None, "stay"

        if self._post_left > 0:
            self._post_left -= 1
            return True, None, "post_roll"
